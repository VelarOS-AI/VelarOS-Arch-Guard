...
import { collectChineseTextFiles, getCachedSourceFile, walk } from './_shared'
...
const requireChineseComments = defineCheck({
  ...
  description:
    '说明性注释与散文式字符串文案必须使用中文（技术词可保留少量英文）；公开 API 的 JSDoc 块豁免。',
  verifies: [
...
function scanComments(
  ...
  exemptPositions: ReadonlySet<number>
): void {
  ...
  while (scanner.scan() !== ts.SyntaxKind.EndOfFileToken) {
    ...
    if (exemptPositions.has(scanner.getTokenPos())) continue
 ...
 */
function collectExportedJsDocPositions(sourceFile: ts.SourceFile): Set<number> {
  const exportedNames = collectNamedExportIdentifiers(sourceFile)
  const positions = new Set<number>()
  for (const statement of sourceFile.statements) {
    if (!isExportedDeclaration(statement, exportedNames)) continue
...
function collectNamedExportIdentifiers(sourceFile: ts.SourceFile): Set<string> {
...
function isExportedDeclaration(statement: ts.Statement, exportedNames: ReadonlySet<string>): boolean {
...
function readDeclarationMembers(statement: ts.Statement): readonly ts.Node[] {
  if (ts.isInterfaceDeclaration(statement) || ts.isClassDeclaration(statement)) return statement.members
  if (ts.isEnumDeclaration(statement)) return statement.members
...
function addJsDocPositions(sourceFile: ts.SourceFile, node: ts.Node, positions: Set<number>): void {
  const ranges = ts.getLeadingCommentRanges(sourceFile.text, node.getFullStart()) ?? []
  for (const range of ranges) {
    if (range.kind !== ts.SyntaxKind.MultiLineCommentTrivia) continue
    if (!sourceFile.text.startsWith('/**', range.pos)) continue
    positions.add(range.pos)
  }
}

function scanStringLiteralNode(
  sourceFile: ts.SourceFile,
  relativePath: string,
  node: ts.Node,
  section: CheckReportSection
): void {
  if (!isInspectableStringLiteral(node)) return
...
function isEnglishProse(value: string): boolean {
  ...
  if (isCodeLikeText(normalized)) return false
  if (/^[A-Z0-9_]+$/.test(normalized)) return false
  if (!/\s/.test(normalized) && /^[\w@./:#-]+$/.test(normalized)) return false
...
function isCodeLikeText(value: string): boolean {
  return (
    /\bimport\s+[\s\S]*\bfrom\b/.test(value) ||
...
function stripTechnicalFragments(value: string): string {
  return value
    .replaceAll(/`[^`]*`/g, ' ')
...
function isInspectableStringLiteral(node: ts.Node): boolean {
  if (!ts.isStringLiteralLike(node) && !ts.isNoSubstitutionTemplateLiteral(node)) return false
  if (isModuleSpecifier(node)) return false
  if (isPropertyName(node)) return false
  if (isOnlyInTypeContext(node)) return false
  return true
}
...
function isOnlyInTypeContext(node: ts.Node): boolean {
