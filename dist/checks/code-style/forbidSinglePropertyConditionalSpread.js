...
const forbidSinglePropertyConditionalSpread = defineCheck({
    ...
    description: '单个可选字段不要写成条件对象展开（`...(c ? { k: v } : {})` / `...(c ? {} : { k: v })` / `...(c && { k: v })`）；直接写 `k: v` / `k: toOptional(v)` / `k: optionalWhen(c, v)`。',
    verifies: [
...
function readSinglePropertyConditionalSpread(node, sourceFile) {
...
function readSpreadSite(node) {
    if (ts.isSpreadAssignment(node) && ts.isObjectLiteralExpression(node.parent))
        return { kind: 'object', node, siblings: node.parent.properties };
    if (ts.isJsxSpreadAttribute(node) && ts.isJsxAttributes(node.parent))
        return { kind: 'jsx', node, siblings: node.parent.properties };
    return undefined;
}
...
function readConditionalShape(expression, sourceFile) {
    if (ts.isConditionalExpression(expression)) {
        ...
        if (whenTrue?.kind === 'single' && whenFalse?.kind === 'empty')
            return { condition: expression.condition, property: whenTrue.property, presentWhenConditionTrue: true };
        if (whenFalse?.kind === 'single' && whenTrue?.kind === 'empty')
            return { condition: expression.condition, property: whenFalse.property, presentWhenConditionTrue: false };
        return undefined;
    }
    // `...(cond && { k: v })`：cond 为假值时展开的是 false / 0 / '' / null / undefined，效果同 `{}`。
    if (ts.isBinaryExpression(expression) && expression.operatorToken.kind === ts.SyntaxKind.AmpersandAmpersandToken) {
        const right = readBranch(expression.right, sourceFile);
        if (right?.kind === 'single')
            return { condition: expression.left, property: right.property, presentWhenConditionTrue: true };
    }
    return undefined;
}
function readBranch(expression, sourceFile) {
    const branch = unwrapExpression(expression).expression;
    if (isEmptyBranch(branch))
        return { kind: 'empty' };
    if (!ts.isObjectLiteralExpression(branch) || branch.properties.length !== 1)
        return undefined;
    ...
    if (ts.isShorthandPropertyAssignment(element)) {
        // `{ k = 1 }` 只在解构里合法；这里不认。
        if (element.objectAssignmentInitializer)
            return undefined;
...
function isEmptyBranch(expression) {
...
function describeReplacement(shape, sourceFile) {
    ...
    if (presentWhenConditionTrue) {
        ...
        if (guard && sameExpressionText(guard.subject, comparable))
            return { valueText: `optionalWhen(${guard.guardName}, ${value})`, exact: true, truthinessSelf: false };
        if (sameExpressionText(stripParens(condition).getText(sourceFile), comparable))
            return { valueText: `toOptional(${value})`, exact: false, truthinessSelf: true };
    }
 ...
 */
function readNullishTest(condition, sourceFile) {
    ...
    if (ts.isCallExpression(expression) && expression.arguments.length === 1) {
        ...
        if (!ts.isIdentifier(callee) || !argument)
            return undefined;
    ...
    if (ts.isBinaryExpression(expression))
        return readNullishComparison(expression, sourceFile);
    return undefined;
}
function readNullishComparison(expression, sourceFile) {
    ...
    for (const [candidate, other] of [
        ...
        if (typeofSubject)
            return { subject: typeofSubject.getText(sourceFile), family: 'undefined', presentWhenTrue: negated };
        if (literal === 'typeof-undefined')
            continue;
...
function readNullishLiteral(expression) {
    ...
    if (expression.kind === ts.SyntaxKind.NullKeyword)
        return 'null';
    if (ts.isStringLiteral(expression) && expression.text === 'undefined')
        return 'typeof-undefined';
    return undefined;
}
/** `typeof v === 'undefined'` 里的 `v`。 */
function readTypeofUndefinedSubject(expression, literal) {
    if (literal !== 'typeof-undefined' || !ts.isTypeOfExpression(expression))
        return undefined;
...
function readTypeGuardSelf(condition, sourceFile) {
    ...
    if (!ts.isIdentifier(callee) || !TypeGuardSelfNames.has(callee.text) || !argument)
        return undefined;
    return { guardName: callee.text, subject: stripParens(argument).getText(sourceFile) };
}
/** 取反条件的源文本：`!x` → `x`，其余包一层 `!(...)`（简单标识符/调用/成员访问不加括号）。 */
function negate(condition, sourceFile) {
    const expression = stripParens(condition);
    if (ts.isPrefixUnaryExpression(expression) && expression.operator === ts.SyntaxKind.ExclamationToken)
        return stripParens(expression.operand).getText(sourceFile);
    const text = expression.getText(sourceFile);
    const simple = ts.isIdentifier(expression) ||
...
function sameExpressionText(left, right) {
...
function readFixBlockers(site, sourceFile, property, hadTypeWrapper) {
    const blockers = [];
    ...
    if (earlierSpread)
        blockers.push('earlier-spread');
    if (property.computed)
        blockers.push('computed');
    if (hadTypeWrapper)
        blockers.push('type-wrapper');
    if (hasComment(site.node, sourceFile))
        blockers.push('comment');
    if (hasSiblingNamed(site, sourceFile, property.nameText))
        blockers.push('duplicate-name');
    if (site.kind === 'jsx' && !jsxAttributeName(property.nameText))
        blockers.push('jsx-name');
    return blockers;
}
function hasComment(node, sourceFile) {
...
function hasSiblingNamed(site, sourceFile, nameText) {
    const target = propertyKey(nameText);
    if (site.kind === 'object')
        return site.siblings.some((sibling) => (ts.isPropertyAssignment(sibling) || ts.isShorthandPropertyAssignment(sibling) || ts.isMethodDeclaration(sibling)) &&
...
function buildAdvice(site, property, replacement, target, blockers) {
    ...
    if (replacement.truthinessSelf)
        parts.push('原写法把 \'\'/0/false 也当成缺席：值可能是这些且原意要丢掉时，保留判断（如 `optionalWhen(isTrue, v)` / `optionalWhen(isPositiveNumber, v)`）。');
    if (!replacement.exact && !replacement.truthinessSelf)
        parts.push('取值依赖条件成立（可选成员、函数调用、有副作用）时改用 `optionalWhenLazy(cond, () => value)`。');
    if (blockers.includes('earlier-spread'))
        parts.push(site.kind === 'jsx'
...
//# sourceMappingURL=forbidSinglePropertyConditionalSpread.js.map